...
async function stateJson(cwd) {
  ...
  const sessions = allSessions(slugForPath(cwd)).slice(0, 20).map((s) => ({
    id: s.id, mtime: s.mtime, title: sessionTitle(s.file),
    source: { kind: s.source.kind, label: s.source.label, hue: HUES[s.source.color] ?? null },
  }));
  return { profiles, sessions, cwd, now: Date.now() };
}

function openTerminal(args) {
  const child = spawn('wt.exe', args, { detached: true, stdio: 'ignore' });
  child.on('error', () => {
    spawn('cmd.exe', ['/c', 'start', '', 'cmd', '/k', ...args], { detached: true, stdio: 'ignore', windowsHide: false }).unref();
  });
  child.unref();
}
...
export function startUi({ port = 7788, open = true, cwd = process.cwd() } = {}) {
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === 'GET' && u.pathname === '/api/state') return json(res, await stateJson(cwd));
      ...
      if (req.method === 'POST') {
        ...
        if (u.pathname === '/api/refresh') {
          ...
          return json(res, await stateJson(cwd));
        }
        ...
        if (u.pathname === '/api/move') {
          ...
          const slug = slugForPath(cwd);
          const found = allSessions(slug).find((s) => s.id === body.id);
          if (!found) return json(res, { error: 'session not found' }, 404);
          copySessionTo(found, body.to, slug);
